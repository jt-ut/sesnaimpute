...
#: The joblib worker cap and the region-chunk size this module reads and
#: measures at once (CODING_RULES.md 10a: an 8 GB resident-memory budget
#: for every worker together; process the thirty regions' sources in
#: chunks rather than holding them all resident).
N_JOBS = 4
REGION_CHUNK_SIZE = N_JOBS
...
def load_field_star_colours(config, region_names):
    ...
    parts = []
    for chunk in _chunks(region_names, REGION_CHUNK_SIZE):
        parts.extend(Parallel(n_jobs=N_JOBS)(delayed(_one_region)(r) for r in chunk))
...
def build(config, regions=None):
    ...
    `REGION_CHUNK_SIZE`, `N_JOBS` at a time (CODING_RULES.md 10a)."""
